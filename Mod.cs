...
public class Mod : ModBase // <= Do not Remove.
{
    ...
    public Mod(ModContext context)
    {
        ...
        //hook scan
        startupScanner.AddMainModuleScan("40 53 48 83 EC ?? 48 8B D9 8B CA E8 ?? ?? ?? ?? 48 85 C0 74 ?? 8B 50 ?? 44 8B C2 41 83 E8 ?? 74 ?? 41 83 F8 ?? 74 ?? 8D 42 ?? 83 F8 ?? 76 ?? 33 D2 41 B8 ?? ?? ?? ?? 48 8B CB 48 83 C4 ?? 5B E9 ?? ?? ?? ?? BA ?? ?? ?? ?? 41 B8 ?? ?? ?? ?? 48 8B CB 48 83 C4 ?? 5B E9", result => // 0x1400FAE34 in Original release of FFTIC
        {
            if (!result.Found)
            {
        ...
        startupScanner.AddMainModuleScan("40 53 48 83 EC ?? 48 8B 05 ?? ?? ?? ?? 89 CB 48 85 C0 75 ?? 48 8B 0D ?? ?? ?? ?? 48 85 C9 74 ?? 8D 50 ?? E8 ?? ?? ?? ?? 48 89 05 ?? ?? ?? ?? 48 85 C0 74 ?? 89 DA 48 89 C1 E8 ?? ?? ?? ?? 49 89 C0 48 85 C0 74 ?? 48 83 38 ?? 7C ?? 48 89 C1 E8 ?? ?? ?? ?? 48 23 1D ?? ?? ?? ?? 48 85 C0 74 ?? 8B 1D ?? ?? ?? ?? 81 EB ?? ?? ?? ?? 49 39 18 7C ?? 4C 89 C1 48 83 C4 ?? 5B E9 ?? ?? ?? ?? 48 8D 05 ?? ?? ?? ?? EB ?? 31 C0 48 83 C4 ?? 5B C3", result => // j_fNex_GetJobCommand in Original release of FFTIC
        {
            if (!result.Found)
            {
        ...
        startupScanner.AddMainModuleScan("40 53 56 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 31 E0 48 89 84 24 ?? ?? ?? ?? 49 63 F8 89 D3 48 89 CE 41 B9 ?? ?? ?? ?? 44 33 0D ?? ?? ?? ?? 4A 8D 14 0C 41 B0 ?? B9 ?? ?? ?? ?? 41 B9 ?? ?? ?? ?? 44 33 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 89 FA B9 ?? ?? ?? ?? 33 0D ?? ?? ?? ?? 4C 8D 04 0C 41 89 D9 48 89 F1 E8 ?? ?? ?? ?? 48 8B 8C 24 ?? ?? ?? ?? 48 31 E1 E8 ?? ?? ?? ?? 48 81 C4 ?? ?? ?? ?? 5F 5E 5B C3", result => // j_fGetJobCommandIconPath in Original release of FFTIC
        {
            if (!result.Found)
            {
